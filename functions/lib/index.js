 ...
 */
exports.callLLM = functions.https.onCall(async (data, context) => {
    ...
    if (!selectedProvider) {
        throw new functions.https.HttpsError("failed-precondition", "No LLM API key configured");
    }
    try {
        let response;
        if (selectedProvider === "gemini" && geminiKey) {
            ...
            throw new functions.https.HttpsError("failed-precondition", `API key not found for provider: ${selectedProvider}`);
        }
        return response;
    }
    catch (error) {
        console.error("LLM API error:", error);
        throw new functions.https.HttpsError("internal", error.message || "Failed to call LLM API");
    }
});
async function callGeminiAPI(messages, apiKey, model, temperature, maxTokens) {
...
async function callOpenAIAPI(messages, apiKey, model, temperature, maxTokens) {
...
async function callAnthropicAPI(messages, apiKey, model, temperature, maxTokens) {
